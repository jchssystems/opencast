...
/**
 * The event endpoint acts as a facade for WorkflowService and Archive providing
 * a unified query interface and result set.
 * <p>
 * This first implementation uses the
 * {@link org.opencastproject.assetmanager.api.AssetManager}. In a later
 * iteration the endpoint may abstract over the concrete archive.
 */
@Path("/")
@RestService(name = "eventservice", title = "Event Service", abstractText = "Provides resources and operations related to the events", notes = {
...
public abstract class AbstractEventEndpoint {
    ...
    public abstract SeriesEndpoint getSeriesService(); // JK#2988
    ...
    public Response deleteEvent(@PathParam("eventId") String id) throws NotFoundException, UnauthorizedException {
        try {
            ...
            // If we couldn't find any trace of the event in the underlying database(s), we
            // can get rid of it
    ...
    public Response getEventGeneralTab(@PathParam("eventId") String id) throws Exception {
        ...
        // List<WorkflowDefinition> workflowsDefinitions =
        // getWorkflowService().listAvailableWorkflowDefinitions();
        // for (WorkflowDefinition wflDef : workflowsDefinitions) {
        // if (wflDef.containsTag(WORKFLOWDEF_TAG)) {
        //
        // actions.add(obj(f("id", v(wflDef.getId())), f("title",
        // v(Opt.nul(wflDef.getTitle()).or(""))),
        // f("description", v(Opt.nul(wflDef.getDescription()).or(""))),
        // f("configuration_panel",
        // v(Opt.nul(wflDef.getConfigurationPanel()).or("")))));
        // }
        // }
    ...
    public Response updateEventScheduling(@PathParam("eventId") String eventId,
        ...
        try {
            ...
            if (schedulingJson.has(SCHEDULING_AGENT_ID_KEY)) {
                ...
                logger.trace("Updating agent id of event '{}' from '{}' to '{}'", eventId,
                        technicalMetadata.getAgentId(), agentId);
            }
            ...
            if (schedulingJson.has(SCHEDULING_START_KEY)) {
                ...
                logger.trace("Updating start time of event '{}' id from '{}' to '{}'", eventId,
                        DateTimeSupport.toUTC(technicalMetadata.getStartDate().getTime()),
            ...
            if (schedulingJson.has(SCHEDULING_END_KEY)) {
                ...
                logger.trace("Updating end time of event '{}' id from '{}' to '{}'", eventId,
                        DateTimeSupport.toUTC(technicalMetadata.getEndDate().getTime()),
            ...
            if (schedulingJson.has(SCHEDULING_AGENT_CONFIGURATION_KEY)) {
                agentConfiguration = Opt
                        .some(JSONUtils.toMap(schedulingJson.getJSONObject(SCHEDULING_AGENT_CONFIGURATION_KEY)));
                logger.trace("Updating agent configuration of event '{}' id from '{}' to '{}'", eventId,
                        technicalMetadata.getCaptureAgentConfiguration(), agentConfiguration);
            }
            ...
            if (schedulingJson.has(SCHEDULING_OPT_OUT_KEY)) {
                ...
                logger.trace("Updating optout status of event '{}' id from '{}' to '{}'", eventId, event.getOptedOut(),
                        optOut);
            }
    ...
    private Event getEventOrThrowNotFoundException(final String eventId)
    ...
    public Response applyAclToEvent(@PathParam("eventId") String eventId, @FormParam("acl") String acl)
        ...
        try {
            ...
            if (eventSource == Source.ARCHIVE) {
                ...
                mediaPackage = getAuthorizationService().setAcl(mediaPackage, AclScope.Episode, accessControlList)
                        .getA();
            ...
            logger.error("Error applying acl '{}' to event '{}' because: {}", accessControlList, eventId,
                    ExceptionUtils.getStackTrace(e));
            return serverError();
        } catch (SchedulerException e) {
    ...
    public Response createEventComment(@PathParam("eventId") String eventId, @FormParam("text") String text,
        ...
        try {
            EventComment createdComment = EventComment.create(Option.<Long>none(), eventId,
                    getSecurityService().getOrganization().getId(), text, author, reason,
    ...
    public Response deleteEventComment(@PathParam("eventId") String eventId, @PathParam("commentId") long commentId)
        ...
        try {
            ...
            logger.error("Unable to delete comment {} on event {}: {}", commentId, eventId,
                    ExceptionUtils.getStackTrace(e));
            throw new WebApplicationException(e);
        }
    }
    ...
    public Response deleteEventCommentReply(@PathParam("eventId") String eventId,
            @PathParam("commentId") long commentId, @PathParam("replyId") long replyId) throws Exception {
        Opt<Event> optEvent = getIndexService().getEvent(eventId, getIndex());
        if (optEvent.isNone())
            return notFound("Cannot find an event with id '%s'.", eventId);
        ...
        try {
            ...
            logger.warn("Could not remove event comment reply {} from comment {}: {}", replyId, commentId,
                    ExceptionUtils.getStackTrace(e));
            throw new WebApplicationException(e);
        }
    }
    ...
    public Response updateEventCommentReply(@PathParam("eventId") String eventId,
            @PathParam("commentId") long commentId, @PathParam("replyId") long replyId, @FormParam("text") String text)
            throws Exception {
        if (StringUtils.isBlank(text))
            return Response.status(Status.BAD_REQUEST).build();
        ...
        try {
            ...
            logger.warn("Could not update event comment reply {} from comment {}: {}", replyId, commentId,
                    ExceptionUtils.getStackTrace(e));
            throw new WebApplicationException(e);
        }
    }
    ...
    public Response createEventCommentReply(@PathParam("eventId") String eventId,
            ...
            @FormParam("resolved") Boolean resolved) throws Exception {
        if (StringUtils.isBlank(text))
            return Response.status(Status.BAD_REQUEST).build();
    ...
    public Response getEventWorkflows(@PathParam("eventId") String id)
        ...
        try {
            if (!optEvent.get().hasRecordingStarted()) {
                ...
                return okJson(obj(
                        f("workflowId",
                                v(agentConfiguration.get(CaptureParameters.INGEST_WORKFLOW_DEFINITION), Jsons.BLANK)),
    ...
    public Response updateEventWorkflow(@PathParam("eventId") String id,
            @FormParam("configuration") String configuration) throws SearchIndexException, UnauthorizedException {
        Opt<Event> optEvent = getIndexService().getEvent(id, getIndex());
        if (optEvent.isNone())
            return notFound("Cannot find an event with id '%s'.", id);
        ...
        if (!optEvent.get().hasRecordingStarted()) {
            ...
            return badRequest(
                    String.format("Event %s workflow can not be updated as the recording already started.", id));
        }
    }
    ...
    public Response getEventErrors(@PathParam("eventId") String eventId, @PathParam("workflowId") String workflowId,
        ...
        // the call to #getEvent should make sure that the calling user has access
        // rights to the workflow
        // FIXME since there is no dependency between the event and the workflow (the
        // fetched event is
        // simply ignored) an attacker can get access by using an event he owns and a
        // workflow ID of
        // someone else.
    ...
    public Response updateEventTransition(@PathParam("eventId") String eventId,
        ...
        try {
            ...
            logger.error("Unable to update transtion {} of event {}: {}", transitionId, eventId,
                    ExceptionUtils.getStackTrace(e));
            throw new WebApplicationException(e, SC_INTERNAL_SERVER_ERROR);
        } catch (ParseException e) {
            // That should never happen
    ...
    @RestQuery(name = "updateAssets", description = "Update or create an asset for the eventId by the given metadata as JSON and files in the body", pathParameters = {
            @RestParameter(name = "eventId", description = "The event id", isRequired = true, type = RestParameter.Type.STRING) }, restParameters = {
                    @RestParameter(name = "metadata", isRequired = true, type = RestParameter.Type.TEXT, description = "The list of asset metadata") }, reponses = {
                            ...
                            @RestResponse(description = "No event with this identifier was found.", responseCode = HttpServletResponse.SC_NOT_FOUND) }, returnDescription = "The workflow identifier")
    public Response updateAssets(@PathParam("eventId") final String eventId, @Context HttpServletRequest request)
            throws Exception {
        try {
            MediaPackage mp = getMediaPackageByEventId(eventId);
    ...
    public Response deleteEventTransition(@PathParam("eventId") String eventId,
        ...
        try {
            ...
            logger.error("Error while trying to delete transition '{}' from event '{}': {}", transitionId, eventId,
                    ExceptionUtils.getStackTrace(e));
            throw new WebApplicationException(e, SC_INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public Response getNewMetadata() {
        ...
        if (optMetadataByAdapter.isSome()) {
            ...
            //metadataList.add(getIndexService().getCommonEventCatalogUIAdapter(), collection); // JK#2988

            // JK#2965
            boolean seriesWithWriteAccess = true;
            if (seriesWithWriteAccess) {
                /*
                // ===== INI getSeriesWriteAccess
                List<JValue> swaList = new ArrayList<>();
                swaList = getSeriesService().getSeriesWriteAccess(null, null, 0, 0, null);
                Map swaMap = new HashMap<>();
                for (Iterator it = swaList.iterator(); it.hasNext();) {
                    JValue v = (JValue) it.next();
                    String jsV = RestUtils.getJsonString(v);
                    org.codehaus.jettison.json.JSONObject jsO = new org.codehaus.jettison.json.JSONObject(jsV);
                    swaMap.put(jsO.getString("title"), jsO.getString("id"));
                }
                */
                // ===== FIN getSeriesWriteAccess

                // /*
                // ===== INI getSeriesWriteAccess2
                Map swaMap = getSeriesService().getSeriesWriteAccess2();
                // */
                // ===== FIN getSeriesWriteAccess2

                Opt<Map<String, String>> mapa = Opt.some(swaMap);
                collection.getOutputFields().get(DublinCore.PROPERTY_IS_PART_OF.getLocalName()).setCollection(mapa);
            }
            // collection.getOutputFields().get(DublinCore.PROPERTY_IS_PART_OF.getLocalName()).getCollection().get().remove("Cities of the world");
            /*
            MetadataField m = collection.getOutputFields().get(DublinCore.PROPERTY_PUBLISHER.getLocalName());
            if (m != null) {
                m.setValue(getSecurityService().getUser().getName());
            }
            */
            metadataList.add(getIndexService().getCommonEventCatalogUIAdapter(), collection); // JK#2988
            // END JK#2965
    ...
    public Response getNewConflicts(@FormParam("metadata") String metadata) throws NotFoundException {
        ...
        try {
                ...
                for (MediaPackage event : events) {
                    ...
                    if (eventOpt.isSome()) {
                        ...
                        if (StringUtils.isNotEmpty(eventId) && eventId.equals(e.getIdentifier()))
                            continue;
                        eventsJSON.add(obj(f("start", v(e.getTechnicalStartTime())),
                                f("end", v(e.getTechnicalEndTime())), f("title", v(e.getTitle()))));
                    } else {
            ...
            logger.error("Unable to find conflicting events for {}, {}, {}: {}", device, startDate, endDate,
                    ExceptionUtils.getStackTrace(e));
            return RestUtil.R.serverError();
        }
    }
    ...
    private List<Field> getCommonElementFields(MediaPackageElement element) {
        ...
        fields.add(f("reference",
                v(element.getReference() != null ? element.getReference().getIdentifier() : null, BLANK)));
        return fields;
    }
     ...
     *
     * @param publications
     *            The elements to pull the data from to create the list of
     *            {@link JValue}s
     * @return {@link List} of {@link JValue}s that represent the
     *         {@link Publication}
     */
    private List<JValue> getEventPublications(Publication[] publications) {
        ...
        for (Publication publication : publications) {
            publicationJSON.add(obj(f("id", v(publication.getIdentifier(), BLANK)),
                    f("channel", v(publication.getChannel(), BLANK)),
                    f("mimetype", v(publication.getMimeType(), BLANK)),
    ...
    private URI signUrl(URI url) {
        if (getUrlSigningService().accepts(url.toString())) {
            try {
                ...
                return URI.create(
                        getUrlSigningService().sign(url.toString(), getUrlSigningExpireDuration(), null, clientIP));
    ...
    public Response workflowAction(@PathParam("eventId") String id, @PathParam("workflowId") String wfId,
            @PathParam("action") String action) throws NotFoundException, UnauthorizedException {
